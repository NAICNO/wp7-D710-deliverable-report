...
### Step 3: Naming the VM and create a key file
...
```{warning}
**Provider selection**: Use **NREC UiO** or **NREC UiB** only. The Google Cloud and IBM Cloud options are experimental and not stable — do not select them.
```
...
Windows 10/11 includes OpenSSH by default, but `chmod` is not available. Fix key permissions via the file properties GUI or PowerShell:
...
The recommended VM configuration for WP7 demonstrators:

| Specification | Value |
|---------------|-------|
| Provider | NREC UiO or NREC UiB |
| VM Type | Virtual machine with GPU (1 x L40S) |
| GPU | NVIDIA L40S (48 GB VRAM) |
| CPU | 16 cores |
| RAM | 64 GB |
| Storage | 100 GB SSD |
| OS | Ubuntu 22.04 LTS |
| Duration | 12–20 hours (workshop), up to 7 days (research) |
